...
/** Menu item with a trailing check mark when selected (theme/priority menus). */
function DropdownMenuCheckItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      className={cn(
        "relative flex cursor-default select-none items-center gap-2 rounded-md px-2 py-1.5 text-sm outline-none transition-colors focus:bg-accent focus:text-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0",
        className,
      )}
      {...props}
    >
      {children}
      {checked ? <Check className="ml-auto size-4" /> : null}
    </DropdownMenuPrimitive.Item>
  );
}
